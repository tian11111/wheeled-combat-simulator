...
> 当前任务仅处于 planning。先决 AC4 未通过时不开始实施。若后续获准实施，逐项记录可复核命令和原始结果；不要把奖励回报代替裁判事件。

1. [ ] 前置门禁：核对 .trellis/tasks/09-25-mujoco-search-targeting/report.md 的 AC4；重跑 dotnet run --project src/Sim.Cli -- match --seed 42 --scenario scenarios/wushu-ring-2026-mujoco.json --events，确认我方真实 BlockScore 与事件归属，并排除 SCORE_BLOCK 掉台循环。未满足则停在这里。
2. [ ] 训练桥：在 Sim.Cli 加本地 JSONL rl-env 请求循环，只支持 reset/step/close；reset 创建新官方 MuJoCo 引擎并 Arm()，step 调用 Tick(usAction, null) 并返回 Gymnasium 所需观测、reward、terminated/truncated 和指标。逐集释放引擎；stdout 仅发协议 JSON，错误写 stderr。Sim.Core、公共回放/Observation DTO 和既有 match 外部控制协议不改。
...
5. [ ] 留出集评测：预先固定 10 个以上训练未用的 seed；evaluate.py 以 deterministic policy 完整跑 MuJoCo 官方场景，并以相同 seed 跑内置 FSM 基线。逐场保存 score/event/fault 指标；通过条件严格按 PRD AC4，真实我方 BlockScore 需能由事件、目标位移与角色归属相互印证。
