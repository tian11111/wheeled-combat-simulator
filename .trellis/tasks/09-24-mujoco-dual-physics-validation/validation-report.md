...
| 项目 | 结果 | 证据 |
|---|---|---|
| 全套测试 | 353/353 通过(旧基线 344 + 新增 MuJoCo 集成 8(含传感器平面投影边界测试)+ 协议 5 + 快照三维姿态等) | `dotnet test RobotSimulator.sln`,还原状态已修复(上次 self-contained 发布失败残留的 `net8.0/win-x64` 目标,经无 RID 还原恢复) |
...
- 物理能力对照(2026-09-25 第三轮修正):专项测试 `NativeMode_MountsTheSixCentimetreStageContinuously` 的恒速倒车(0.6 m/s × 120 tick)同样**只把车体抬上台沿后卡住**(最高中心 Z≈0.122、车体倾斜 ~0.36 rad、四角从未全部上台),并未完成完整登台。即当前未标定参数(轮半径 0.065 m、轮驱动力上限 0.3)下,无论 FSM 还是简单恒速倒车都无法把四轮全部送上 6 cm 台面;登台适配(已派发的新任务)需调整轮驱参数或登台机动,本任务不改。测试断言的"无穿透、无位置跳跃"仍然成立。
- 判定:不属于 AC2 违规(AC2 要求的可重复登台/掉台/推块场景由直接驱动测试提供,覆盖抬升接触与非跳跃下台);属于必须报告的模式差异。修复路径是真机遥测标定(既有任务 08-28)或登台适配,本任务不改 FSM、不放宽测试掩盖。
- 传感器一致性边界(设计如此,R3):首轮传感器仍为解析模型——`SensorSampler` 消费车体平面投影 (x,y,th) 与 `_field` 台面/围栏几何,不做 MuJoCo raycast。已知不一致:①车体俯仰/侧倾不改变传感器安装位姿(按平放车体解析计算);②台沿/悬空判定用台面矩形而非 MuJoCo 接触;③方块对距离传感器是 2D 圆形投影,不反映 3D 姿态。针对性边界测试已补(`NativeMode_SensorChannelsTrackAnalyticPlaneProjectionWhileBodyTilts`):零噪声下车体被抬上台沿并倾斜期间,ir_ground 通道逐 tick **精确**等于台面矩形平面投影的三元值——锁定"传感器不感知三维姿态"契约,若未来升级三维感知该测试会失败提示同步修订文档。仍仅文档化未测试固化:方块 2D 圆投影、俯仰对安装位姿的影响。
...
| AC2/R2 可重复场景 | ✅ | 直行/转向、车车不穿透、推块出界、6 cm 台沿抬升接触、掉台/下台:8 项集成测试;无非有限状态/穿透/跳跃(测试断言);登台完整性的缺口见 §4.1 修正 |
