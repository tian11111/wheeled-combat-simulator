...
编辑器只能修改**起始布局**（场地位姿/出发区/车辆出生点/能量块固定坐标），
不向运行中的仿真注入瞬移或外力；应用后由新 `Scenario` 重建会话。

| 操作 | 方式 |
| --- | --- |
| 进入 / 退出编辑 | E（未应用的修改退出后不生效） |
| 选择对象 | 鼠标点击场地整体 / 黄蓝出发区 / 能量块 / 双方小车（顶部检视栏显示选中文本: 我方小车、对手小车等） |
| 拖动 | 按住左键拖动（出发区/能量块/小车为场局部坐标, 场地为世界平移; 小车只改出生位置, 朝向与出发区保持不动） |
...
实体点选走解析命中代理（能量块 = 世界对齐盒, 小车 = 以该角色碰撞半径 + 少量输入
容差的直立代理）, 不依赖"把鼠标射线投到 y=0 再猜目标", 低视角点击可见的方块/小车
同样命中; 点空白处取消选择。没有实体命中时才回退到出发区/场地的地面拾取。编辑器
激活期间相机让出全部指针处理, 拖动事件标记为已处理。
