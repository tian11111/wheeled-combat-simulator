...
| 操作 | 行为 |
| --- | --- |
| 左键拖动 | 转动视角: 概览/跟随绕焦点环绕（偏航 +0.3°/px, 俯仰 10°–85° 限幅 +0.25°/px）, 俯视绕视线轴自旋（俯仰固定 -90°）; F5/场地重建时回默认角 |
...
台面纹理逐像素采样内核同一 `FieldGrayLocal(x, y)` 的**区域几何**（传感器 0–1000 语义:
走道 0、边缘黑带 300、斜坡至中心白、中央 0.6×0.6 红区 650），按**官方效果图调色板**
显示: 走道官方深灰、擂台外圈黑边渐入白心（亮度 = (g−300)/700）、中央红区红底白“武”；
底座侧面白。传感器 0–1000 数值本身不变——调色板是视觉约定，区域几何永远来自内核。
像素↔场局部坐标的轴向契约集中在纯 C# `FieldGrayTextureMap`（图像行 0 = 场地南侧、
列 0 = 西侧, `Sim.Tests` 可无头断言），材质为 `Unshaded`——有向光/高光不得在台面
制造灰度梯度。未标定的坐标无关灰度 CSV 永不进入运行时。
